...
public class ProductDao extends BaseDao {
	...
	public List<Products> listAllkm(int amount) {
		String sql = "SELECT  * FROM product where product_discount !=0 ORDER BY product_id OFFSET " + amount
	...
	public List<Products> GetDataProductsPaginate(int category_id, int start, int totalPage) {
		...
		if (ListProductsByID.size() > 0) { // neu nhu co san pham moi lay du lieu phan trang
			String sql = "SELECT  * FROM product where product_danhmuc=" + category_id + " "
					+ " ORDER BY product_danhmuc OFFSET " + start + "-1 " + " ROWS " + " FETCH NEXT " + totalPage + " "
					+ " ROWS ONLY";
	...
	public List<Products> GetDataProductsPaginatetangdan(int category_id, int start, int totalPage) {
		...
		if (ListProductsByID.size() > 0) { // neu nhu co san pham moi lay du lieu phan trang
			String sql = "SELECT  * FROM product where product_danhmuc=" + category_id
					+ " ORDER BY  product_price ASC, product_danhmuc OFFSET " + start + "-1 " + " ROWS "
	...
	public List<Products> GetDataProductsPaginategiamdan(int category_id, int start, int totalPage) {
		...
		if (ListProductsByID.size() > 0) { // neu nhu co san pham moi lay du lieu phan trang
			String sql = "SELECT  * FROM product where product_danhmuc=" + category_id
					+ " ORDER BY  product_price DESC, product_danhmuc OFFSET " + start + "-1 " + " ROWS "
					+ " FETCH NEXT " + totalPage + " " + " ROWS ONLY";
			ListProduct = _jdbcTemplate.query(sql, new MapperProducts());
		}
	...
	public List<Products> GetDataProductsPaginatekhuyenmai(int category_id, int start, int totalPage) {
		...
		if (ListProductsByID.size() > 0) { // neu nhu co san pham moi lay du lieu phan trang
			String sql = "SELECT  * FROM product where product_danhmuc = " + category_id
					+ " ORDER BY  product_discount DESC, product_danhmuc OFFSET " + start + "-1 " + " ROWS "
					+ " FETCH NEXT " + totalPage + " " + " ROWS ONLY";
			ListProduct = _jdbcTemplate.query(sql, new MapperProducts());
		}
	...
	public List<Products> GetDataProductsPaginatebanchay(int category_id, int start, int totalPage) {
		...
		if (ListProductsByID.size() > 0) { // neu nhu co san pham moi lay du lieu phan trang
			String sql = "SELECT  * FROM product where  product_danhmuc = " + category_id
					+ " ORDER BY product_purchased DESC, product_danhmuc OFFSET " + start + "-1 " + " ROWS "
					+ " FETCH NEXT " + totalPage + " " + " ROWS ONLY";
			ListProduct = _jdbcTemplate.query(sql, new MapperProducts());
		}
	...
	public List<Products> GetDataProductsPaginatenew(int category_id, int start, int totalPage) {
		...
		if (ListProductsByID.size() > 0) { // neu nhu co san pham moi lay du lieu phan trang
			String sql = "SELECT  * FROM product where product_danhmuc = " + category_id
					+ " ORDER BY product_id DESC, product_danhmuc OFFSET " + start + "-1 " + " ROWS " + " FETCH NEXT "
					+ totalPage + " " + " ROWS ONLY";
			ListProduct = _jdbcTemplate.query(sql, new MapperProducts());
		}
	...
	public boolean save(Products pro) {
		...
		try {
			...
			new ConnectionToDB().excuteSql("insert into product values(" + pro.getProduct_id() + ", N'"
					+ pro.getProduct_name() + "'," + pro.getProduct_price() + "," + pro.getProduct_discount() + ","
					+ pro.getProduct_danhmuc() + ",'" + pro.getProduct_image() + "'," + pro.getProduct_view() + ","
					+ pro.getProduct_purchased() + "," + pro.getProduct_trangthai() + "," + pro.getProduct_soLuongMua()
					+ ",N'" + pro.getProduct_content() + "'," + pro.getProduct_soluongtonkho() + ",N'"
					+ pro.getProduct_thuonghieu() + "',N'" + pro.getProduct_sanxuat() + "',N'"
					+ pro.getProduct_thanhphan() + "',N'" + pro.getProduct_nongdocon() + "',N'"
					+ pro.getProduct_luongga() + "',N'" + pro.getProduct_luongduong() + "',N'"
					+ pro.getProduct_thetich() + "',N'" + pro.getProduct_baoquan() + "',N'" + pro.getProduct_sudung()
					+ "','" + pro.getProduct_nsx() + "','" + pro.getProduct_hsd() + "'," + pro.getNhanvien_id() + ","
					+ pro.getMaNCC() + ")");
			return true;
		} catch (Exception e) {
			e.printStackTrace();
